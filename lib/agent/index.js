...
class Agent {
   ...
   * @private
   * @param   {Number} shard The ID of the shard that's ready.
   */
  _onShardReady (shard) {
    console.log(`Connected as ${this._client.user.username} on shard ${shard}`)

    if (this._statusMessage) {
      if (typeof this._statusMessage === 'function') this._statusMessage(this._client.shards.get(shard).editStatus, this, shard)
    ...
    if (this._dblAPI) this._dblAPI.postStats(this._client.guilds.size, shard, this._client.shards.size).catch(this._onError)
  }
 ...
 * @param    {Eris.Client.setStatus} setStatus      The setStatus function from the Eris client.
