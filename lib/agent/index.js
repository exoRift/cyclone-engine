  ...
  async buildHelp ({ description, supportServerInviteCode, color, prefixImage, version, page = 1 }) {
    ...
    if (this._replacers && this._replacers.length) {
      fields.push('**Replacers:**\n*Inserts live data values into commands. `|REPLACERNAME|`*\n\n' +
        this._replacers.reduce((a, r) => `${a}${a.length ? '\n' : ''}${r.info}`, ''))
    }
    if (this._reactCommands && this._reactCommands.length) {
      fields.push('**React Commands:**\n*React to any message with the appropriate reaction to trigger its command.*\n\n' +
        this._reactCommands.reduce((a, rC) => `${a}${a.length ? '\n' : ''}${rC.info}`, ''))
    }

    if (page > fields.length) page = fields.length
    if (page < 1) page = 1
    ...
    const embed = {
      ...
      fields: [
        {
          name: `Commands page ${page} out of ${fields.length}`,
          value: fields[page - 1]
        }
      ],
      footer: {
