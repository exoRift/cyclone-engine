...
class Agent {
   ...
   * @prop  {function(msg: Eris.Message, res: CommandResults): *}                                     [data.agentOptions.postMessageFunction]        A function that runs after every message whether it triggers a command or not.
  ...
  _bindEvents () {
    ...
    if (this._fireOnEdit) this._client.on('messageUpdate', this._onMessage)
    this._client.on('messageReactionAdd', this._onReaction)
    if (this._fireOnReactionRemove) this._client.on('messageReactionRemove', this._onReaction)
    this._client.on('shardReady', this._onShardReady)
    this._client.on('shardDisconnect', this._onShardDisconnect)
  }
