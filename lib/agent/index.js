...
class Agent {
   ...
   * @prop  {Eris}                         [data.Eris=Eris]                                   The Eris class the system runs off of
   ...
   * @prop  {String[]}                     [data.options.intents=[]]                          Additional intents to subscribe to for the gateway. (Intents are automatically calculated from agent features getting enabled. However, add additional intents if needed) (Eris supports snakeCase if CAMEL_CASE is not preferred) {@link https://discord.com/developers/docs/topics/gateway#list-of-intents}
   ...
   * @prop  {Object}                       [data.options.postEventActions={}]                 A collection of functions to run after an event is triggered
   * @prop  {PostMessageAction}            [data.options.postEventActions.message]            A function that runs after every message whether it triggers a command or not
   * @prop  {PostReactionAction}           [data.options.postEventActions.reaction]           A function that runs after every reaction whether it triggers a react command or not
   * @prop  {PostPermissionAction}         [data.options.postEventActions.permission]         A function that runs after a permission is changed
   * @prop  {PostPrefixAction}             [data.options.postEventActions.prefix]             A function that runs after a guild prefix is changed
   ...
   */
  constructor ({ Eris = require('eris'), token, handlerData = {}, options = {} }) {
    const {
      commands,
      reactCommands,
      replacers,
     ...
     * @prop    {Object}                       postEventActions            A collection of functions to run after an event is triggered.
     * @prop    {PostMessageFunction}          postEventActions.message    A function that runs after every message whether it triggers a command or not
     * @prop    {PostReactionFunction}         postEventActions.reaction   A function that runs after every reaction whether it triggers a react command or not
     * @prop    {PostPermissionFunction}       postEventActions.permission A function that runs after a permission is changed
     * @prop    {PostPrefixFunction}           postEventActions.prefix     A function that runs after a guild prefix is changed
     */
    this._options = {
      statusMessage,
      fireOnEdit,
  ...
  _checkPermissions (msg, member, command) {
    if (!msg.channel.type && command.options.authLevel) {
      const level = this.getAuthLevel(member)
  ...
  getAuthLevel (user) {
    ...
    if (user.permissions.has('administrator')) return Infinity

    return user.roles.reduce((a, r) =>
      (this._guildData[user.guild.id] && this._guildData[user.guild.id].permissions && this._guildData[user.guild.id].permissions[r] > a)
        ? this._guildData[user.guild.id].permissions[r]
        : a, 0)
  }
  ...
  _initHandlers () {
    if (this._handlerData.commands) {
      console.info('Initializing Command Handler')
    ...
    if (this._handlerData.reactCommands) {
      console.info('Initializing Reaction Handler')
   ...
   */
  _onError (error) {
    console.error('An internal Eris error has occured:\n', error)
  }
  ...
  async _onMessage (msg) {
    ...
    return this.commandHandler.handle(msg)
      .then((res) => {
        if (this._options.postEventActions.message) this._options.postEventActions.message(msg, res)
  ...
  async _onReaction (msg, emoji, userID) {
    ...
    return this.reactionHandler.handle(msg, emoji, user)
      .then((res) => {
        if (this._options.postEventActions.reaction) this._options.postEventActions.reaction(msg, emoji, user, res)
  ...
  _onShardDisconnect (err, shard) {
    if (err) console.error(`Shard ${shard} lost connection. Error:\n${err}`)
  }
  ...
  setGuildPrefix (guild, prefix) {
    ...
    if (this._options.postEventActions.prefix) this._options.postEventActions.prefix(guild, prefix, oldPrefix)

    if (this.commandHandler) this.commandHandler.setGuildPrefix(guild, prefix)
  }

  /**
   * Update the auth level of a guild role
   * @param   {String} guild The ID of the guild
   * @param   {String} role  The role to modify
   * @param   {Number} level The auth level to set the role to
   * @returns {Object}       All current auth levels of the guild
   */
  updateAuthLevel (guild, role, level) {
    ...
    if (this._guildData[guild] && this._guildData[guild].permissions) {
      if (Object.entries(this._guildData[guild].permissions).find((p) => p[0] !== role && p[1] === parsedLevel)) console.warn(`WARNING: Multiple roles in guild '${guild}' have been assigned level ${level}`)
    } else {
    ...
    if (this._options.postEventActions.permission) this._options.postEventActions.permission(guild, role, level, oldLevel)
