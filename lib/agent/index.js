...
class Agent {
   ...
   * @prop      {Number}                [data.color=0]     An integer color that changes the sidebar color of the embed
  ...
  _checkPermissions (msg, member, command) {
    if (!msg.channel.type && command.options.authLevel && this._guildData.permissions) {
  ...
  getTopPermissionLevel (user) {
    ...
    return user.roles.reduce((a, r) => (this._guildData.permissions[user.guild.id] && this._guildData.permissions[user.guild.id][r] > a) ? this._guildData.permissions[user.guild.id][r] : a, 0)
  }
    ...
    if (this._handlerData.commands) {
      ...
      this.commandHandler = new CommandHandler({
        ...
        options: {
          ...
          guildPrefixes: this._guildData.prefixes,
          _app: this.app
        }
      })
    }
  ...
  setGuildPrefix (guild, prefix) {
    this._guildData.prefixes[guild] = prefix
  ...
  updatePermission (guild, role, level) {
    ...
    if (this._guildData.permissions[guild] && Object.entries(this._guildData.permissions[guild]).find((p) => p[0] !== role && p[1] === parsedLevel)) console.log(`WARNING: Multiple roles in guild '${guild}' have been assigned level ${level}`)

    this._guildData.permissions[guild] = {
      ...this._guildData.permissions[guild],
      [role]: parsedLevel
    }

    return this._guildData.permissions[guild]
  }
}
