...
class Agent {
   ...
   * @prop  {Object}                       [data.options.serverOptions={}]                    Options for specific servers (Usually initially loaded from a database)
   * @prop  {ServerPermissionList}         [data.options.serverOptions.permissions={}]        Initial permissions for roles of servers
   * @prop  {PrefixList}                   [data.options.serverOptions.prefixes={}]           Initial prefixes for servers
  ...
  constructor ({ Eris, token, handlerData = {}, options = {} }) {
     ...
     * @type {Object}
     * @prop {ServerPermissionList} permissions Permission levels for roles in guilds
     * @prop {PrefixList}           prefixes    Custom server-side prefixes
     */
    this.serverData = {
      permissions,
      serverPrefixes: prefixes
    }
   ...
   * @param  {Eris.Member}          member  The subject member of the server
   * @param  {Command|ReactCommand} command The command
   * @throws {InputError}
   */
  _checkPermissions (msg, member, command) {
    if (!msg.channel.type && command.options.authLevel && this.serverData.permissions) {
  ...
  async connect (_count = 1) {
    if (_count <= this._options.connectRetryLimit) {
      ...
      console.error('RECONNECTION LIMIT REACHED; RECONNECTION CANCELED')
  ...
  getTopPermissionLevel (user) {
    ...
    return user.roles.reduce((a, r) => (this.serverData.permissions[user.guild.id] && this.serverData.permissions[user.guild.id][r] > a) ? this.serverData.permissions[user.guild.id][r] : a, 0)
  }
        ...
        if (this._handlerData.commands) {
          ...
          this.commandHandler = new CommandHandler({
            ...
            options: {
              ...
              serverPrefixes: this.serverData.serverPrefixes
            }
          })
        }
  ...
  updatePermission (guild, role, level) {
    ...
    if (this.serverData.permissions[guild] && Object.entries(this.serverData.permissions[guild]).find((p) => p[0] !== role && p[1] === parsedLevel)) console.log(`WARNING: Multiple roles in guild '${guild}' have been assigned level ${level}`)

    this.serverData.permissions[guild] = {
      ...this.serverData.permissions[guild],
      [role]: parsedLevel
    }

    return this.serverData.permissions[guild]
  }
}
...
/**
 * A list of prefixes for each server supplied to the Agent. It is mapped with a server's ID equalling its prefix
 * @typedef {Object} PrefixList
 * @prop    {Prefix} serverID
 * @example                     { '463886367496339458': ';' }
 */

/**
 * A list of permissions for each role of a server. It is mapped with a role's ID equalling its permission level
 * @typedef {Object} PermissionList
 * @prop    {Number} roleID
 */

/**
 * A list of role permissions for each server supplied to the Agent. It is mapped with a server's ID equalling a PermissionList
 * @typedef {Object}         ServerPermissionList
 * @prop    {PermissionList} serverID
 */
