...
class Agent {
   ...
   * @prop  {postMessageFunction}          [data.agentOptions.postMessageFunction]         A function that runs after every message if it triggers a command.
  ...
  _onShardReady (shard) {
    ...
    if (this._dblAPI) this._dblAPI.postStats(this._client.guilds.size, shard, this._client.shards.size).catch((err) => this._onError(err))
  }
