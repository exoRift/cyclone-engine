...
class Agent {
   ...
   * @prop  {Object}                       [data.options.initialPermissions={}]
  ...
  constructor ({ Eris, token, handlerData = {}, options = {} }) {
    ...
    /**
     * Permission levels for roles in servers
     * @type {Object}
     */
    ...
    /**
     * An array of user IDs that are not allowed to use bot commands
     * @type {String[]}
     */
    this.userBlacklist = []
  ...
  /**
   * Add users to the blacklist that are not allowed to use bot command
   * @param   {String[]|String} users The IDs of the users that should be blacklisted
   * @returns {String[]}              All currently blacklisted users
   */
  blacklistUsers (users) {
    if (!Array.isArray(users)) users = [users]

    for (const user of users) if (!this.userBlacklist.includes(user)) this.userBlacklist.push(user)
  ...
  /**
   * Get a server member's top authority level role
  ...
  getTopPermissionLevel (user) {
    if (!user.roles) throw Error('A user was provided when a member instance is required')

    return user.roles.reduce((a, r) => this.permissions[user.guild][r] > a ? this.permissions[user.guild][r] : a, 0)
  }
  ...
  async _onMessage (msg) {
    if (msg.author.bot || this.userBlacklist.includes(msg.author.id)) return

    if (this.commandHandler) {
      return this.commandHandler.handle(msg, this.getTopPermissionLevel(msg.author))
        .then((res) => {
  ...
  async _onReaction (msg, emoji, userID) {
    ...
    if (user.bot || this.userBlacklist.includes(user.id)) return

    if (this.reactionHandler) {
      return this.reactionHandler.handle(msg, emoji, user, this.getTopPermissionLevel(user))
        .then((res) => {
  ...
  /**
   * Update the permission level of a server role
   * @param   {String} guild The ID of the server
   * @param   {String} role  The role to modify
   * @param   {Number} level The permission level to set the role to
   * @returns {Object}       All current permissions of the server
   */
  updatePermission (guild, role, level) {
    if (!this.client.guilds.has(guild)) throw Error('An ID to a server the bot isn\'t in was provided. ID: ' + guild)
    if (!this.client.guilds.roles.has(role)) throw Error('An ID to a role that doesn\'t exist was provided. ID: ' + role)

    level = parseInt(level)
    if (isNaN(level)) throw Error(`'${level}' is not a valid level. A level must be a valid integer`)

    if (Object.values(this.permissions[guild]).includes(level)) console.log(`WARNING: Two roles in guild '${guild}' have been assigned level ${level}`)
   ...
   * @param   {String[]|String} users The IDs of the users that should be removed from the blacklist
   * @returns {String[]}              All currently blacklisted users
   */
  whitelistUsers (users) {
    ...
    for (const user of users) {
      const index = this.userBlacklist.findIndex((u) => u === user)

      if (index !== -1) this.userBlacklist.splice(index, 1)
    }
