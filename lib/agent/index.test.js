...
test('buildHelp', async (t) => {
  const helpContent = {
    author: {
      name: 'client 1 Help',
      icon_url: 'https://raw.githubusercontent.com/exoRift/cyclone-engine/master/assets/Help Icon.png'
    },
...
test('userBlacklist', (t) => {
  ...
  const blacklistedUser = t.context.agent.client._addUser({ id: '2' }, t.context.guilds.dGuild)
...
test('permissionsSystem', (t) => {
  ...
  return t.context.agent._onReady().then(async () => {
    ...
    t.throws(() => t.context.agent.getTopPermissionLevel(secondUser), 'A user was provided when a member instance is required', 'Provided user not member errors')
  })
})
