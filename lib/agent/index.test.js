...
test('postMessageFunction', async (t) => {
  await t.context.init({
    ...
    postEventFunctions: {
      message: (msg, { results: [{ responses: [{ content }] }] }) => console.log(msg.channel.id + ' ' + content)
    }
  })
...
test('postReactionFunction', async (t) => {
  await t.context.init({
    ...
    postEventFunctions: {
      reaction: (msg, emoji, user, { results: [{ responses: [{ content }] }] }) => console.log(`${msg.channel.id} ${emoji.name} ${user.id} ${content}`)
    }
  })
