...
test.beforeEach((t) => {
  ...
  t.context.init = (handlerData, agentOptions) => {
    ...
    t.context.users = {
      otherUser: t.context.agent._client._addUser({ id: '1' })
    }

    t.context.guilds = {
      dGuild: t.context.agent._client._joinGuild({ guildData: { id: '0' }, channels: [{ id: '0' }] })
    }
    ...
    t.context.channels.restrictedChannel._setPermission(t.context.agent._client.user.id, 'sendMessages', false)
  }
})
...
test('statusEdited', async (t) => {
  ...
  for (let s = 0; s < supplies.length; s++) {
    ...
    const spy = sinon.spy(t.context.agent._client.shards.get(0), 'editStatus')

    t.context.agent._client._setConnectStatus(true)
...
test('connectRetryLimit', async (t) => {
  ...
  const spy = sinon.spy(t.context.agent._client, 'connect')
...
test('messageEvent', (t) => {
  ...
  return t.context.agent._onReady(t.context.agent._client).then(async () => {
    const messages = {
      proper: new PseudoClient.Message(t.context.channels.dChannel, '!c1', undefined, t.context.users.otherUser),
      bot: new PseudoClient.Message(t.context.channels.dChannel, '!c1', undefined, t.context.agent._client.user),
    ...
    const spy = sinon.spy(t.context.agent._commandHandler, 'handle')

    t.context.agent._client.emit('messageCreate', messages.proper)
    t.true(spy.calledWith(messages.proper), 'Proper message')

    t.context.agent._client.emit('messageCreate', messages.bot)
...
test('buildHelp', async (t) => {
  ...
  for (let s = 0; s < supplies.length; s++) {
    ...
    for (const section in supplies[s]) {
      const {
        embed,
        pages
      } = await t.context.agent.buildHelp(helpData, !s ? undefined : i)
  ...
  const lowerPage = await t.context.agent.buildHelp(helpData, 0)
  helpContent.fields = [{
  ...
  const higherPage = await t.context.agent.buildHelp(helpData, 2)
  ...
  const multipleCommandPages1 = await t.context.agent.buildHelp(helpData, 1)
  const multipleCommandPages2 = await t.context.agent.buildHelp(helpData, 2)
...
test('ErisErrorRecievedEvent', (t) => {
  ...
  t.context.agent._client.emit('error', error)
...
test('disconnection', async (t) => {
  ...
  const statusSpy = sinon.spy(t.context.agent._client.shards.get(0), 'editStatus')
  ...
  t.context.agent._client.emit('shardDisconnect', undefined, 0)
...
test('fireOnEdit', async (t) => {
  ...
  t.context.agent._client.emit('messageUpdate', message)
...
test('reactionEvent', (t) => {
  ...
  return t.context.agent._onReady(t.context.agent._client).then(async () => {
    ...
    const spy = sinon.spy(t.context.agent._reactionHandler, 'handle')

    const properEmoji = await message.addReaction('🍕')
    t.context.agent._client.emit('messageReactionAdd', message, properEmoji, t.context.users.otherUser.id)
    t.true(spy.calledWith(message, properEmoji, t.context.users.otherUser), 'Proper message')

    const botEmoji = await message.addReaction('🍧')
    await t.context.agent._onReaction(message, botEmoji, t.context.agent._client.user.id)
    t.false(message.channel.createMessage.calledWith({
      content: 'ice cream',
      embed: undefined
...
test('fireOnReactionRemove', (t) => {
  ...
  return t.context.agent._onReady(t.context.agent._client).then(async () => {
    const handlerSpy = sinon.spy(t.context.agent._reactionHandler, 'handle')

    const message = t.context.channels.dChannel.createMessage('hello')

    t.context.agent._client.emit('messageReactionRemove', message, {
      name: '🍕'
    }, t.context.users.otherUser.id)

    t.true(handlerSpy.calledWith(message, {
      name: '🍕'
    }, t.context.users.otherUser))
  })
})

test('userBlacklist', (t) => {
  t.context.init({
    commands: baseCommand,
    reactCommands: baseReactCommand
  }, {
  ...
  const blacklistedUser = t.context.agent._client._addUser({ id: '2' })

  return t.context.agent._onReady(t.context.agent._client).then(async () => {
    const commandHandlerSpy = sinon.spy(t.context.agent._commandHandler, 'handle')
    const reactionHandlerSpy = sinon.spy(t.context.agent._reactionHandler, 'handle')

    const message = await t.context.channels.dChannel.createMessage('!c1')
    const secondMessage = await t.context.channels.dChannel.createMessage('!c1')

    message.author = t.context.users.otherUser
    await t.context.agent._onMessage(message)
    t.true(commandHandlerSpy.calledWith(message), 'Good user message')

    secondMessage.author = blacklistedUser
    await t.context.agent._onMessage(secondMessage)
    t.false(commandHandlerSpy.calledWith(secondMessage), 'Blacklisted user message')

    const reaction = await message.addReaction('🍕')

    await t.context.agent._onReaction(message, reaction, t.context.users.otherUser.id)
    t.true(reactionHandlerSpy.calledWith(message, reaction, t.context.users.otherUser), 'Good user reaction')

    await t.context.agent._onReaction(message, reaction, blacklistedUser.id)
    t.false(reactionHandlerSpy.calledWith(message, reaction, blacklistedUser), 'Blacklisted user reaction')

    commandHandlerSpy.restore()
    reactionHandlerSpy.restore()
  })
})
...
test('attachmentSystem', (t) => {
  t.context.init({
    commands: new Command({
      ...
      action: ({ attachments }) => attachments.attachment
    }),
  ...
  return t.context.agent._onReady().then(async () => {
    ...
    t.is((await t.context.agent._commandHandler.handle(await t.context.channels.dChannel.createMessage('!testattachment'))).results[0].responses[0].content, 'hello', 'Command handler')

    const message = await t.context.channels.dChannel.createMessage('message')

    t.is((await t.context.agent._reactionHandler.handle(message, await message.addReaction('🍕'))).results[0].responses[0].content, 'hello', 'Reaction handler')
  })
})
...
test('removingAttachments', (t) => {
  ...
  t.is(t.context.agent._attachments.attachment, 'hello', 'Attachment added')

  t.context.agent.removeAttachment('attachment')

  t.is(t.context.agent._attachments.attachment, undefined, 'Attachment removed')
})
