...
test.beforeEach((t) => {
  ...
  t.context.init = (handlerData, agentOptions) => {
    ...
    t.context.users = {
      otherUser: t.context.agent.client._addUser({ id: '1' })
    }
...
test('reactionEvent', (t) => {
  ...
  return t.context.agent._onReady(t.context.agent.client).then(async () => {
    ...
    await t.context.agent._onReaction(message, botEmoji, t.context.agent.client.user.id)
...
test('fireOnReactionRemove', (t) => {
  ...
  return t.context.agent._onReady(t.context.agent.client).then(async () => {
    const handlerSpy = sinon.spy(t.context.agent.reactionHandler, 'handle')
...
test('userBlacklist', (t) => {
  ...
  t.context.agent.blacklistUsers('2')

  const blacklistedUser = t.context.agent.client._addUser({ id: '2' })

  return t.context.agent._onReady(t.context.agent.client).then(async () => {
    const commandHandlerSpy = sinon.spy(t.context.agent.commandHandler, 'handle')
    const reactionHandlerSpy = sinon.spy(t.context.agent.reactionHandler, 'handle')
    ...
    t.context.agent.whitelistUsers('2')

    t.deepEqual(t.context.agent.userBlacklist, [], 'Successful whitelist')
  })
})

test('blacklistDuplicates', (t) => {
  t.context.init()

  t.context.agent.blacklistUsers(['2', '2'])

  t.deepEqual(t.context.agent.userBlacklist, ['2'], 'Duplicate not added')

  t.context.agent.whitelistUsers(['3', '2'])
