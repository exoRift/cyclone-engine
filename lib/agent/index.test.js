...
import PseudoClient, { User } from '../../test/pdc.js'
...
test('connectRetryLimit', async (t) => {
  ...
  return t.context.agent.connect().catch((err) => {
    t.is(err.message, 'Limit reached.', 'Error message thrown')
...
test('permissionsSystem', (t) => {
  ...
  t.deepEqual(t.context.agent.serverData.permissions, initialPermissions, 'Inital permissions set')
