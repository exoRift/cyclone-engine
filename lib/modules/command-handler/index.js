...
class CommandHandler {
  ...
  async handle (msg) {
    ...
    return Promise.all(responses).then(this._successfulResponses.bind(this, { msg, command, awaited }))
  }
   ...
   */
  _successfulResponses ({ msg, command, awaited }, results) {
    for (const { options, response } of results) {
      ...
      if (wait) {
        ...
        this._addAwait({ channel: msg.channel.id, user: msg.author.id, rsp: response, wait })
      }

      if (reactInterface) {
        if (!response) throw Error('Cannot attach an interface to a non-existent response.')
    ...
    if (awaited) {
      if (awaited.refreshAfterUse) awaited.refresh()
      else awaited.clear()
    }
 ...
 * @prop    {Command}        CommandHandlerResults.command                        The object of the command called.
