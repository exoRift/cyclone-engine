...
class CommandHandler {
   ...
   * @returns {Eris.Message|Error<responseError>}                        The response that was utilized.
   */
  ...
  _parseArgs (command, chars) {
    ...
    for (let i = 0; i < command.options.args.length; i++) {
      const delim = command.options.args[i].delim || ' '

      for (let j = parsed.join('').length; j < chars.length; j++) {
        const char = chars[j]

        if (char === delim[0] && chars.substring(j, chars.length).startsWith(delim.substring(1, delim.length)) && i !== command.options.args.length - 1) {
      ...
      if (command.options.args[i].type === 'number' && parsed[i] !== undefined) {
