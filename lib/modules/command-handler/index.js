...
class CommandHandler extends BaseHandler {
  ...
  constructor ({ agent, client, commands, replacers, options = {} }) {
    ...
    if (replacers && open.startsWith(prefix)) console.warn('WARNING: Your replacer opening brace starts with your prefix. This could lead to some issues.')
  ...
  _loadCommand (command) {
    ...
    if (command.options.args.length) {
      ...
      if (lastArg.delim !== ' ') console.warn(`DISCLAIMER: Command ${command.name}'s last argument unnecessarily has a delimiter`)
      if (mandDiag.invalid) console.warn(`WARNING: Command ${command.name} has invalid argument mandates.\nMake sure all arguments leading up to the last mandatory argument are mandatory as well.`)
    }
