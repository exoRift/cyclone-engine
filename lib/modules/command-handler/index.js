...
class CommandHandler extends BaseHandler {
   ...
   */
  async handle (msg) {
    // Normalize message content (replacers, prefix, bot mention)
    ...
    // Get command
    let args = normalized.content.split(' ')
    let keyword
    ...
    args = this._parseArgs(msg.channel.guild, command, args.join(' '))
   ...
   * @returns {Array<String|Number|Eris.Channel|Eris.Member>}         The parsed arguments
   */
  _parseArgs (guild, command, chars) {
    ...
    for (let a = 0; a < command.options.args.length; a++) {
      ...
      if (parsed[a] !== undefined) {
        switch (command.options.args[a].type) {
          case 'number':
          ...
          case 'user':
          case 'channel': {
            const value = this._parseDynArg(guild, parsed[a], command.options.args[a].type)

            if (value) parsed[a] = value
            else return
  ...
  _parseDynArg (guild, arg, type) {
    const container = type === 'user' ? guild.members : guild.channels

    const match = arg.match(this._regexes.dynamics[type])

    if (match) return container.get(match[1])
    else return container.find((i) => (type === 'user' ? i.username : i.name).toLowerCase() === arg.toLowerCase())
  }
  ...
  _runReplacers (guild, content) {
    return content.replace(new RegExp(`\\${this._options.replacerBraces.open}(.+?)\\${this._options.replacerBraces.close}`, 'g'), (content, capture) => {
      ...
      if (replacer) {
        args = this._parseArgs(guild, replacer, args.join(' '))
   ...
   */
  _normalize (msg) {
    const guildPrefix = this._options.guildPrefixes[msg.channel.guild.id]
