...
class CommandHandler extends BaseHandler {
   ...
   */
  _parseArgs (guild, command, chars) { // issue: extra spaces
    const parsed = []

    let startingIndex = 0
    for (let arg = 0; arg < command.options.args.length; arg++) {
      const delim = command.options.args[arg].delim || ' '

      for (let charIndex = startingIndex; charIndex <= chars.length; charIndex++) {
        const potenDelim = chars.substring(charIndex, charIndex + delim.length)

        if ((potenDelim === delim && arg !== command.options.args.length - 1) || charIndex >= chars.length) {
          startingIndex = charIndex + delim.length
      ...
      if (parsed[arg] !== undefined) {
        switch (command.options.args[arg].type) {
          case 'number':
            const number = Number(parsed[arg])
    ...
    return content.replace(new RegExp(`${this._options.replacerBraces.open.replace(this._regexes.escapables, '\\$&')}(.+?)${this._options.replacerBraces.close.replace(this._regexes.escapables, '\\$&')}`, 'g'), (content, capture) => {
      ...
      if (replacer) {
        ...
        if (replacer.options.args.length) {
          args = this._parseArgs(guild, replacer, capture.substring(keyword.length))
