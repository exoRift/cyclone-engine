...
class CommandHandler {
  ...
  async handle (msg) {
    ...
    if (typeof command.action !== 'function') throw Error('Command action is invalid:\n' + (command instanceof Await ? 'awaitID: ' + command._id : command.name))
  ...
  _parseArgs (command, args) {
    ...
    for (let i = 0; i < command.args.length; i++) {
      ...
      if (command.args[i].type === 'number' && cleaned[i] !== undefined) {
        const number = parseInt(cleaned[i])

        if (number) cleaned[i] = number
        else return
      }
    }
    return cleaned
