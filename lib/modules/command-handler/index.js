...
class CommandHandler extends BaseHandler {
  ...
  _getAwait (msg, normalized) {
    ...
    if (wait) {
      if (wait.options.requirePrefix) console.log(normalized)
  ...
  _normalize (msg) {
    ...
    const prefixRegex = new RegExp(`^(?:<@!?${this._client.user.id}> ?|${(guildPrefix || this._options.prefix).replace(regexEscapablesRegex, '\\$&')})(.+?)$`)

    const postReplacers = this._runReplacers(msg.content)
