...
class CommandHandler {
   ...
   */
  _generateError (name, message, code, Type) {
    const compiledMsg = `${name} [${code}]: ${message}`

    if (Type) return new Type(name, compiledMsg, code)
    else {
      const err = Error(compiledMsg)
  ...
  async handle (msg) {
    ...
    if (!awaited) {
      ...
      if (text.startsWith(this._options.prefix)) {
        text = text.substring(this._options.prefix.length)
      } else if (!msg.channel.type) return
    }
    ...
    if (command.options.serverOnly && msg.channel.type) throw new InputError('This command has been disabled outside of server text channels', 'Trying using it in a server channel', 'nonserver')
    if (command.options.authLevel && this._agent.permissions) {
      const level = this._agent.getTopPermissionLevel(msg.author)
