...
class CommandHandler {
  ...
  async _successfulResponse ({ msg, command }, options, response) {
    ...
    if (wait) await this._addAwait({ channel, user: msg.author.id, initResponse: response, wait })
   ...
   * @prop    {Eris.Message}   data.initResponse The response to the command that created the await.
