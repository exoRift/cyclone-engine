...
test.serial('aliases', async (t) => {
  t.is((await handler.handle(await dChannel.createMessage('!aliastest'))).results[0].responses[0].content, 'content', 'Base name ran')

  t.is((await handler.handle(await dChannel.createMessage('!alias'))).results[0].responses[0].content, 'content', 'Alias ran')
})
