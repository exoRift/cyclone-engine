...
test.serial('prefixDetermination', async (t) => {
  ...
  t.is((await handler.handle(await dChannel.createMessage('!c1'))).results[0].response.content, 'command1', 'Command 1 ran')
  ...
  t.is((await fakeHandler.handle(await dChannel.createMessage('<>c1'))).results[0].response.content, 'command1', 'Custom prefix')
})

test.serial('mentionPrefix', async (t) => {
  t.is((await handler.handle(await dChannel.createMessage(`<@${client.user.id}>c1`))).results[0].response.content, 'command1', 'No space')

  t.is((await handler.handle(await dChannel.createMessage(`<@${client.user.id}> c1`))).results[0].response.content, 'command1', 'With space')
})
...
test.serial('commandDiscrimination', async (t) => {
  ...
  t.truthy(command1)
  t.is(command1.results[0].response.content, 'command1', 'Command 1 ran')

  t.is((await handler.handle(await dChannel.createMessage('!c2'))).results[0].response.content, 'command2', 'Command 2 ran')
...
test.serial('aliases', async (t) => {
  t.is((await handler.handle(await dChannel.createMessage('!aliastest'))).results[0].response.content, 'content', 'Base name ran')

  t.is((await handler.handle(await dChannel.createMessage('!alias'))).results[0].response.content, 'content', 'Alias ran')
})
...
test.serial('multipleResponses', async (t) => {
  ...
  t.truthy(dChannel.messages.find((m) => m.content === 'channel1'), 'Channel 1')

  t.truthy(otherChannel.messages.find((m) => m.content === 'channel2'), 'Channel 2')
})
...
test.serial('ignoreCodes', async (t) => {
  const invalidChannel = new Command({
    name: 'invalidchannel',
  ...
  const fakeHandler = new CommandHandler({
    ...
    client,
    ownerID: botOwner.id,
    commands: invalidChannel,
    options: {
      ignoreCodes: [50008]
    }
  })

  t.deepEqual(await fakeHandler.handle(await dChannel.createMessage('!invalidchannel')), {
    command: invalidChannel,
    results: [{
      options: {
        channel: nonTextChannel.id
      },
...
test.serial('emptyAction', async (t) => {
  const {
    command,
    results
  } = await handler.handle(await dChannel.createMessage('!emptyaction'))
  const {
    options,
    response
  } = results[0]

  t.deepEqual(command, mockCommands.find((c) => c.name === 'emptyaction'))
  t.deepEqual(options, {})
...
test.serial('partialReturnObject', async (t) => {
  ...
  t.deepEqual(emptyResults, [], 'Empty object')
...
test.serial('noSendMessagePermission', async (t) => {
  t.deepEqual(await handler.handle(await dChannel.createMessage('!testmessagepermissions')), {
    command: mockCommands.find((c) => c.name === 'testmessagepermissions'),
    results: [{
      options: {
        channel: restrictedChannel.id
      }
  ...
  const otherMessage = await dChannel.createMessage('!c1')

  dChannel._setPermission(client.user.id, 'sendMessages', false)

  t.deepEqual(await handler.handle(otherMessage), {
    command: mockCommands.find((c) => c.name === 'c1'),
    results: [{
      options: {}
    }]
  })
...
test.serial('reactInterfaces', async (t) => {
  ...
  return t.throwsAsync(noReactionHandler.handle(await dChannel.createMessage('!reactinterface')), {
    instanceOf: Error,
    message: 'The reaction handler isn\'t enabled; enable it by passing an empty array to Agent.reactCommands.'
  }, 'No reaction handler')
})
...
test.serial('argumentSystem', async (t) => {
  t.is((await handler.handle(await dChannel.createMessage('!argstest hello there'))).results[0].response.content, 'hello+there+undefined-undefined', 'Only mandatory args')
  ...
  t.is((await handler.handle(await dChannel.createMessage('!argstest hello there good|sir'))).results[0].response.content, 'hello+there good+sir-undefined', 'Custom delim')
  ...
  t.is((await handler.handle(await dChannel.createMessage('!argstest hello there good|sir 5'))).results[0].response.content, 'hello+there good+sir-5', 'Number arg success')
...
test.serial('replacerSystem', async (t) => {
  t.is((await handler.handle(await dChannel.createMessage('!echo h|r1| l |r2|'))).results[0].response.content, 'hreplacer1 l replacer2', 'Replacer differenciation')

  t.is((await handler.handle(await dChannel.createMessage('!echo h|invalid| l |r2|'))).results[0].response.content, 'hINVALID KEY l replacer2', 'Invalid Replacer')

  t.is((await handler.handle(await dChannel.createMessage('!echo hello |r3 1| there'))).results[0].response.content, 'hello replacer3 2 there', 'Replacer with args')

  t.is((await handler.handle(await dChannel.createMessage('!echo hello |r3| there'))).results[0].response.content, 'hello INVALID ARGS there', 'Incorrect args')
  ...
  t.is((await fakeHandler.handle(await dChannel.createMessage('!echo h<r1> l <r2>'))).results[0].response.content, 'hreplacer1 l replacer2', 'Custom replacer braces')
})
...
test.serial('databaseRequesting', (t) => {
  return _prepareTable().then(async () => {
    ...
    t.is((await handler.handle(dMessage)).results[0].response.content, '0', 'User 0')

    return t.is((await handler.handle(otherMessage)).results[0].response.content, '1', 'User 1')
  })
})

test.serial('awaitSystem', async (t) => {
  t.is((await handler.handle(await dChannel.createMessage('!awaittest'))).results[0].response.content, 'first', 'Any message pt. 1')
  ...
  t.is(awaitedResults.results[0].response.content, 'second', 'Any message pt. 2')

  t.is((await handler.handle(await dChannel.createMessage('!awaittest check'))).results[0].response.content, 'first', 'Wrong user pt. 1')
  ...
  t.is((await handler.handle(await dChannel.createMessage('!awaittest check'))).results[0].response.content, 'first', 'Conditional await fail pt. 1')
  t.is(await handler.handle(await dChannel.createMessage('!runawait different')), undefined, 'Conditional await fail pt. 2')

  t.is((await handler.handle(await dChannel.createMessage('!awaittest check'))).results[0].response.content, 'first', 'Conditional await success pt. 1')
  t.is((await handler.handle(await dChannel.createMessage('!runawait check'))).results[0].response.content, 'second', 'Conditional await success pt. 2')

  t.is((await handler.handle(await dChannel.createMessage('!awaittest check 100'))).results[0].response.content, 'first', 'Timeout await fail pt.1')
  await delay(200)
  t.is(await handler.handle(await dChannel.createMessage('!runawait check')), undefined, 'Timeout await fail pt.2')

  t.is((await handler.handle(await dChannel.createMessage('!awaittest check 2000'))).results[0].response.content, 'first', 'Timeout await success pt.1')
  t.is((await handler.handle(await dChannel.createMessage('!runawait check'))).results[0].response.content, 'second', 'Timeout await success pt.2')

  t.is((await handler.handle(await otherChannel.createMessage('!awaittest check 2000'))).results[0].response.content, 'first', 'oneTime await fail pt.1')
  ...
  t.is((await handler.handle(await otherChannel.createMessage('!awaittest check 2000'))).results[0].response.content, 'first', 'oneTime await success pt.1')
  t.is((await handler.handle(await otherChannel.createMessage('!runawait check'))).results[0].response.content, 'second', 'oneTime await success pt.2')

  t.is((await handler.handle(await dChannel.createMessage('!testawaitrefresh'))).results[0].response.content, 'first', 'refreshOnUse pt. 1')
  t.is((await handler.handle(await dChannel.createMessage('trigger'))).results[0].response.content, 'second', 'refreshOnUse pt. 2')
  t.is((await handler.handle(await dChannel.createMessage('trigger'))).results[0].response.content, 'second', 'refreshOnUse pt. 3')
...
test.serial('restrictedCommands', async (t) => {
  ...
  t.is((await handler.handle(ownerMsg)).results[0].response.content, 'restricted message', 'Successful grant')
})
