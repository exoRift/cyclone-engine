...
restrictedChannel._setPermission(client.user.id, 'sendMessages', false)
...
test.serial('multiCharacterDelim', async (t) => {
  return t.is((await handler.handle(await dChannel.createMessage('!longdelim first multi||third'))).results[0].responses[0].content, 'first multi third')
})
...
test.serial('awaitSystem', async (t) => {
  ...
  t.is((await handler.handle(await dChannel.createMessage('!testmultipleawaits'))).results[0].responses[0].content, 'first', 'multiple awaits pt. 1')
  t.is((await handler.handle(await dChannel.createMessage('trigger'))).results[0].responses[0].content, 'second', 'multiple awaits pt. 2')
  t.is((await handler.handle(await otherChannel.createMessage('trigger'))).results[0].responses[0].content, 'second', 'multiple awaits pt. 3')
})

test.serial('awaitNoArrayDirectly', (t) => {
  return handler.addAwaits(new Await({
    options: {
      user: client.user.id,
      channel: dChannel.id
    },
    ...
    const wait = handler._awaits.get('00')
...
test.serial('providedAwaitUser', async (t) => {
  t.is((await handler.handle(await dChannel.createMessage('!awaitusertest'))).results[0].responses[0].content, 'initial', 'Initial message')

  t.is(await handler.handle(await dChannel.createMessage('trigger')), undefined, 'Wrong user did not trigger')

  const correctUserMsg = await dChannel.createMessage('trigger')
  correctUserMsg.author = botOwner

  t.is((await handler.handle(correctUserMsg)).results[0].responses[0].content, 'triggered', 'Initial message')
})

test.serial('awaitRequirePrefix', async (t) => {
  t.is((await handler.handle(await dChannel.createMessage('!requireprefixtest'))).results[0].responses[0].content, 'initial', 'Initial message')

  t.is(await handler.handle(await dChannel.createMessage('trigger')), undefined, 'No prefix failure')

  t.is((await handler.handle(await dChannel.createMessage('!trigger'))).results[0].responses[0].content, 'success', 'Prefix success')
})
...
test.serial('awaitNoFallbackErrors', async (t) => {
  await t.throwsAsync(handler.addAwaits(new Await({
    options: {
      user: client.user.id
    },
    action: () => 'hello'
