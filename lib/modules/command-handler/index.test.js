...
test('invalidChannel', async (t) => {
  ...
  return t.throwsAsync(t.context.handler.handle(await t.context.channels.dChannel.createMessage('!invalidchannel')), IgnoredError, 'DiscordRESTError [50035]: Invalid Form Body\n  channel_id: Value "undefined" is not snowflake.')
})
...
test.serial('deleteAfter', async (t) => {
  ...
  return t.throwsAsync(t.context.handler.handle(await t.context.channels.dChannel.createMessage('!invaliddeleteafter')), {
    instanceOf: Error,
...
test('awaitNoFallbackErrors', (t) => {
  ...
  t.throws(() => t.context.handler.addAwaits(new Await({
    ...
    action: () => 'hello'
  })), Error, 'An await didn\'t have a defined channel or fallback channel. This can be caused by directly calling CommandHandler.addAwaits', 'No channel')
  ...
  t.throws(() => t.context.handler.addAwaits(new Await({
    ...
    action: () => 'hello'
  })), Error, 'An await didn\'t have a defined user or fallback user. This can be caused by directly calling CommandHandler.addAwaits', 'No user')
})
...
test('noPrefixForDM', async (t) => {
  ...
  t.is((await t.context.handler.handle(await t.context.channels.dChannel.createMessage('c1'))).results[0].responses[0].content, 'command1')
})
...
test('serverOnly', async (t) => {
  ...
  t.is((await t.context.handler.handle(await t.context.channels.dChannel.createMessage('!serveronlytest'))).results[0].responses[0].content, 'content', 'Successful in server')

  t.context.channels.dChannel.type = 1

  return t.throwsAsync(t.context.handler.handle(await t.context.channels.dChannel.createMessage('!serveronlytest')), {
    name: 'This command has been disabled outside of server text channels',
