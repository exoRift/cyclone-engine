...
class ReactionHandler {
  ...
  async bindInterface (msg, reactInterface) {
    ...
    if (!reactInterface.designatedUsers) reactInterface.designatedUsers = [msg.author.id]
  ...
  async handle (msg, emoji, user) {
    ...
    if (command.options.restricted) return
    if (command.options._designatedUsers && !command.options._designatedUsers.includes(user.id)) return
    if (command.options.serverOnly && msg.channel.type) throw new InputError('This reaction command has been disabled outside of server text channels', 'Trying using it in a server channel', 'nonserver')
