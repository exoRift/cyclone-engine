...
class ReactionHandler {
  ...
  async handle (msg, emoji, user) {
    ...
    for (const commandResult of commandResults) {
      ...
      if (commandResult.options) commandResult.options.channel = channel
      ...
      if (!channelObject.permissionsOf(this._client.user.id).has('sendMessages')) {
        results.push({ options: commandResult.options || {} })
      ...
      if (content || embed || file) {
        ...
        results.push(channelObject.createMessage({ content, embed }, file)
          .catch((err) => {
            if (err.code === 50035 && (err.message.includes('length'))) {
              ...
              }).then((response) => { return { options: commandResult.options || {}, response } })
            }
          ...
          .then((response) => { return { options: commandResult.options || {}, response } }))
      } else if (commandResult.options) results.push({ options: commandResult.options })
    }

    return Promise.all(results).then(this._successfulResponses.bind(this, { msg, user, command, foundReactInterface: command.parentInterface }))
  }
