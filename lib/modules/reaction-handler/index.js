...
class ReactionHandler {
  ...
  async handle (msg, emoji, user) {
    ...
    const command = this._getInterface.get(msg, emoji) || this._reactCommands.get(emoji.name)
    ...
    if (typeof command.action !== 'function') throw Error('ReactCommand action is invalid:\n' + command.emoji)
  ...
  _successfulResponses ({ msg, user, command, foundReactInterface }, results) {
    for (const { options, response } of results) {
      ...
      if (wait) this._agent._commandHandler._addAwait({ channel: msg.channel.id, user: user.id, response, wait })
  ...
  async bindInterface (msg, reactInterface) {
    if (!msg) throw Error('Cannot attach an interface to a non-existent message.')
