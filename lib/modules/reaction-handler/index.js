...
class ReactionHandler {
  ...
  async handle (msg, emoji, user) {
    ...
    const _successfulResponse = (rsp) => {
      ...
      return { command, msg: { content, embed, file }, options: { wait, reactInterface, deleteAfter }, rsp }
    }
    ...
    if (content || embed || file) {
      ...
      return this._client.createMessage(channel, { content, embed }, file)
        .catch((err) => {
          if (err.code === 50035 && (err.message.includes('length') || err.message.includes('size'))) {
            return this._.createMessage(client, 'Text was too long, sent as a file instead.', {
