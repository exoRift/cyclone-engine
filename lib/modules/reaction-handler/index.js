...
class ReactionHandler {
  ...
  async _successfulResponse ({ msg, user, command, parentInterface }, options, response) {
    ...
    if (awaits) {
      if (this._agent._commandHandler) await this._agent._commandHandler.addAwaits(awaits, { _fallBackChannel: channel, _fallBackUser: user.id, _triggerResponse: response })
