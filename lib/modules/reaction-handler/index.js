...
class ReactionHandler {
  ...
  async bindInterface (msg, reactInterface) {
    ...
    if (!reactInterface.options.designatedUsers) reactInterface.options.designatedUsers = [msg.author.id]
  ...
  async handle (msg, emoji, user) {
    ...
    if (command.options.serverOnly && msg.channel.type) return
    if (command.options.authLevel && this._agent.permissions) {
      const level = this._agent.getTopPermissionLevel(user)
