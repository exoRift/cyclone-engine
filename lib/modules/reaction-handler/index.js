...
class ReactionHandler {
  ...
  async handle (msg, emoji, user) {
    ...
    const _successfulResponse = (rsp) => {
      ...
      if (command._removeReaction) msg.removeReaction(user.id, command.emoji)

      if (reactInterface && reactInterface._deleteAfterUse) {
        const emojis = []
        const buttons = reactInterface.buttons.values()
        for (const button of buttons) emojis.push(button.emoji)

        msg.removeReactions(this._client.id, emojis)
