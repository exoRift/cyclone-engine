...
class ReactionHandler {
   ...
   * @prop    {Eris.Message|Error<MinorError>}         results.response         The Eris message that was sent to Discord.
   * @returns {Promise<Eris.Message|Error<MinorError>}                          Overall results from the react command.
   */
  async _successfulResponse ({ msg, user, command, parentInterface }, options, response) {
    ...
    if (wait) {
      if (this._agent._commandHandler) await this._agent._commandHandler._addAwait({ channel, user: user.id, initResponse: response, wait })
    ...
    if (parentInterface && parentInterface.deleteAfterUse) {
      const buttons = parentInterface.buttons.values()
 ...
 * @prop    {Array<Eris.Message|Error<MinorError>>} ReactionHandlerResults.results.response               The response sent to Discord.
 */
