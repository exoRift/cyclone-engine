...
class ReactionHandler {
  ...
  constructor ({ agent = {}, client, ownerID, reactCommands = [], options = {} }) {
     ...
     * @private
     * @type    {Object}
     * @prop    {Number[]} ignoreCodes The Discord error codes to ignore.
    ...
    if (content || embed || file) {
      ...
      return channel.createMessage({ content, embed }, file)
        .catch((err) => {
          ...
          if (this._options.ignoreCodes.includes(err.code)) return Error('ignoredError')
   ...
   * @returns {Eris.Message|Error<responseError>}                          Overall results from the react command.
   */
  _successfulResponse ({ msg, user, command, parentInterface }, options, response) {
    const {
      channel,
      awaits,
      reactInterface,
    ...
    if (reactInterface) this.bindInterface(response, reactInterface)
   ...
   * @returns {Object}                         The resulting interface data.
   */
  bindInterface (msg, reactInterface) {
   ...
   * @returns {ReactInterface}     The detached interface.
   */
  detachInterface (msg) {
