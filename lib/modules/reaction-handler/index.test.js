...
test.serial('reactCommandDescrimination', async (t) => {
  ...
  t.is(pizza.results[0].response.content, 'pizza', 'Pizza ran')

  const iceCream = await handler.handle(t.context.message, await t.context.message.addReaction('🍨'), botOwner)
  t.is(iceCream.results[0].response.content, 'ice cream', 'Ice cream ran')

  const unknownReactCommand = await handler.handle(t.context.message, await t.context.message.addReaction('🍟'), botOwner)
  t.is(unknownReactCommand, undefined, 'Unknown react command')

  t.truthy(t.context.message.channel.messages.find((m) => m.content === 'pizza' || m.content === 'ice cream'), 'Message sent')
})

// test.serial('aliases', async (t) => {
//   t.is((await handler.handle(await t.context.message.channel.createMessage('!aliastest'))).results[0].response.content, 'content', 'Base name ran')

//   t.is((await handler.handle(await t.context.message.channel.createMessage('!alias'))).results[0].response.content, 'content', 'Alias ran')
// })
