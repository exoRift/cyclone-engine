...
test.serial('reactCommandDescrimination', async (t) => {
  ...
  t.is((await handler.handle(t.context.message, await t.context.message.addReaction('🍨'), botOwner)).results[0].responses[0].content, 'ice cream', 'Ice cream react command ran')
...
test.serial('restrictedReactCommands', async (t) => {
  ...
  t.is(authorSuccess.results[0].responses[0].content, 'hamburger', 'Message author only failure')
  ...
  t.is(designatedSuccess.results[0].responses[0].content, 'candy', 'Designated users success')
...
test.serial('databaseRequesting', async (t) => {
  return _prepareTable().then(async () => {
    ...
    t.is(first.results[0].responses[0].content, '0', 'User 0')
    t.is(second.results[0].responses[0].content, '1', 'User 1')
  })
})
...
test.serial('interfaceSystem', async (t) => {
  ...
  await fakeHandler.bindInterface(t.context.message, new ReactInterface({
    buttons: [
      new ReactCommand({
        emoji: ':emoji:123',
        action: () => 'button'
...
test.serial('detachNonexistentInterface', async (t) => {
  t.is(await handler.detachInterface('123'), undefined)
})
...
test.serial('deleteAfterUseReactionRemovalError', async (t) => {
  const {
    results: [{ responses: [response] }]
  } = await handler.handle(t.context.message, await t.context.message.addReaction('🍗'), botOwner)
