...
test('designatedUserReactCommands', async (t) => {
  const designatedCommand = new ReactCommand({
    emoji: '🍧',
    desc: 'Testing designated users',
    action: () => 'ice cream'
  })
  designatedCommand.options._designatedUsers = ['1', '3']
  ...
  const adminFailure = await t.context.handler.handle(t.context.message, await t.context.message.addReaction('🍕'), t.context.users.otherUser)
  t.is(adminFailure, undefined, 'Admin only failure')

  const adminSuccess = await t.context.handler.handle(t.context.message, await t.context.message.addReaction('🍕'), t.context.users.botOwner)
  t.is(adminSuccess.results[0].responses[0].content, 'pizza', 'Admin only success')
...
test('interfaceSystem', async (t) => {
  ...
  t.deepEqual((await t.context.handler.bindInterface(t.context.message, new ReactInterface({
    buttons: baseCommand,
    options: {
      designatedUsers: ['1']
    }
  }))).options.designatedUsers, ['1'], 'Designated users inheritance')
  ...
  return t.throwsAsync(t.context.handler.handle(t.context.message, await t.context.message.addReaction('🍔'), t.context.users.botOwner), 'Cannot attach an interface to a non-existent message or response.', 'Attempted to add a react interface to non-existent response')
})
...
test('dontRemoveButtonsInDetach', async (t) => {
  const spy = sinon.spy(t.context.message, 'removeReaction')
  ...
  await t.context.handler.detachInterface(t.context.message, false)
...
test.serial('deleteAfter', async (t) => {
  ...
  return t.throwsAsync(t.context.handler.handle(t.context.message, await t.context.message.addReaction('🍟'), t.context.users.botOwner), 'Cannot delete a non-existent response with a delay of:\n5', 'Attempted to add a delete delay to non-existent response')
})
...
test('awaits', async (t) => {
  ...
  await t.throwsAsync(t.context.handler.handle(t.context.message, await t.context.message.addReaction('🍕'), t.context.users.botOwner), 'The command handler isn\'t enabled; enable it by passing an empty array to Agent.handlerData.commands', 'No reaction handler')

  t.context.handler._agent = {
    commandHandler,
    validateChannel: () => true
  }
  commandHandler._agent = t.context.handler._agent

  await t.context.handler.handle(t.context.message, await t.context.message.addReaction('🍕'), t.context.users.botOwner)
  t.truthy(commandHandler._awaits.get(`${t.context.message.channel.id}${t.context.users.botOwner.id}`), 'Await registered')
})
...
test('guildOnly', async (t) => {
  ...
  t.is((await t.context.handler.handle(t.context.message, await t.context.message.addReaction('🍕'))).results[0].responses[0].content, 'content', 'Successful in guild')

  t.context.channels.dChannel.type = 1

  t.is(await t.context.handler.handle(t.context.message, await t.context.message.addReaction('🍕')), undefined, 'No response in DM')
})
...
test('DMChannel', async (t) => {
  ...
  const message = await channel.createMessage('c1')

  await t.context.handler.handle(message, await message.addReaction('🍕'), t.context.users.otherUser)

  t.truthy(channel.messages.find((m) => m.content === 'pizza'))
})
